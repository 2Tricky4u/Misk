...
- **Tests:** there is no external (`dotnet test`) harness — verification runs **in-engine via dev console commands** (defined in `Code/Debug/`, dev-only, safe to delete). Press Play, then type into the game/editor console:
  ...
  - `misk_reload` — force-reload the static JSON data into the running game. **Run after any `Assets/data/**.json` or data-model edit:** `StaticData._current` is a `static` cache that survives hot-reload *and* Play restarts, so newly-added fields read as null/empty until reloaded (this is what makes pawns/factions render stale after a JSON change).
...
5. **Everything player-facing is data, not code.** Fantasy names, colors, adjacency, region bonuses, dice counts, starting armies, and theme all live in `Assets/data/**.json`, loaded and validated by `Code/Data/GameDataLoader.cs` + `MapValidator.cs`. Never hardcode a territory/faction/region name or a balance number in logic. New map/faction/rule = new/edited JSON; `MapValidator` fail-fasts on bad adjacency/refs/duplicates. This extends to **presentation-only** data too: e.g. the army-denomination pieces (infantry=1 / cavalry=5 / artillery=10) are defined by `theme.armyTokens` (`dark_atlas.json`) + a per-faction `units` map (`factions.json`), decomposed for rendering by the engine-free `Code/Presentation/UnitStack.cs`. Garrisons stay a single `Territory.Armies` int — the tokens are a **pure visual** decomposition with the numeric badge authoritative, so no domain/test change.
...
- **Stale static data after a data-model change.** `StaticData.Current` caches `GameData` in a `static` field that outlives hot-reload and Play restarts. After editing `Assets/data/**.json` or a DTO/model, the running game keeps the old objects (new fields read null) — run `misk_reload` to refresh. SCSS hot-reloads independently of C#, so a half-applied change can look like a render bug; recompile the C# (focus the editor, Play stopped) before diagnosing.
- **Razor UI gotchas (s&box-specific).** Two that bite: (1) a conditional `onclick` on a component's own `<root>` is *not* cleared when the attribute is absent next render — the stale handler keeps firing. Put click handlers on **child** elements under a non-interactive wrapper root. (2) Deal-in/enter animations use the `:intro` idiom (`:intro` sets the start state for one frame, then the element's base `transition` animates it); a `@keyframes`/`animation` on `:intro` does **not** fire. Also prefer named structs over named value-tuples in markup — the source generator is fragile with tuple element access.
